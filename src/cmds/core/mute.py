...
class MuteCog(commands.Cog):
    ...
    async def mute(
        ...
        if member is None:
            return await ctx.respond(f"{user.mention} is not in this server.")
    ...
    @has_any_role(*settings.role_groups.get("ALL_ADMINS"), *settings.role_groups.get("ALL_MODS"))
    async def unmute(self, ctx: ApplicationContext, user: discord.Member) -> Interaction | WebhookMessage:
